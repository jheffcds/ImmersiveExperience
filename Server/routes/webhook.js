  ...
  async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      try {
        ...
        if (!userId || !Array.isArray(sceneIds) || sceneIds.length === 0) {
          return res.status(400).send('Missing userId or sceneIds in metadata');
        }
        ...
        for (const scene of newScenes) {
          await new Purchase({
            ...
            scene: scene._id,
            price: scene.price,
            receiptEmail: session.customer_email || user.email
          }).save();
        }
        ...
        if (newScenes.length > 0) {
                    ...
                    <tr>
                      <td style="padding: 30px;">
                        ...
                        <ul style="padding-left: 20px; line-height: 1.6;">
                          ${newScenes.map(scene => `<li><strong>${scene.title}</strong></li>`).join('')}
                        </ul>
          ...
          console.log(`✅ ${newScenes.length} new scene(s) added to user ${user.email}`);
        }
      } catch (err) {
